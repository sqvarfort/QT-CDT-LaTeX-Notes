...
\end{description}
\subsubsection{Fermion ground-state wavefunction}
\begin{description}
...
\beq
\left( c_{k \uparrow}^\dagger \right)^2 \ket{0} = 0
\eeq
...
\beq
\mathcal{N}  = \prod_k \sqrt{1 + N_p |\varphi_k|^2}
\eeq
where
\beq
v_k = \sqrt{N_p } \varphi_k u_k = \sqrt{\frac{N_p \varphi_k^2}{1 + N_p \varphi_k^2}}
\eeq

\item[Fermionic BEC state] using the normalisation, we have
\beq
\ket{\Psi_{BEC} } = \prod_k \left( u_k + v_k c_{k \uparrow}^\dagger c_{- k \downarrow} ^\dagger \right) \ket{0}
\eeq
...
In this section, we find the ground-state of the fermionic system by guessing the wavefunction and showing that it minimises the free energy $\mathcal{F}$. 
\begin{description}
\item[Many-body Hamiltonian for attractive interactions] given by 

\beq
H = \sum_{k, \sigma} \epsilon_k c_{k \sigma}^\dagger c_{k \sigma}  + \frac{U_0}{V} \sum_{k, k', q} c^\dagger_{k + \frac{q}{2} \uparrow} c^\dagger_{- k + \frac{q}{2} \downarrow} c_{k' + \frac{q}{2} \downarrow} c_{- k ` + \frac{q}{2} \uparrow}
\eeq
...
\beq
\mathcal{F}  = \braket{ H - \mu N} = \sum_k 2 \xi_k v_k^2 + \frac{U_0}{V} \sum_{k , k' } u_k v_k u_{k'} v_{k'}
\eeq
...
\beq
\Delta = - \frac{U_0}{V} \sum_k u_k v_k = - \frac{U_0}{V} \sum_k \frac{\Delta}{2 E_k}
\eeq
...
\beq
H = \sum_{k \sigma} \epsilon_k c_{k \sigma}^\dagger c_{k \sigma} + \frac{U_0}{V} \sum_{k, k'} c_{k \uparrow}^\dagger c_{- k \downarrow}^\dagger c_{k'\downarrow} c_{k'\uparrow}
\eeq
...
\beq
C_k = \braket{c_{k \uparrow} c_{- k \downarrow} }
\eeq
...
\beq
H - \mu N = \sum_k E_k \left( \gamma_{k \uparrow}^\dagger \gamma_{k \uparrow} + \gamma_{k \downarrow}^\dagger k_{k \downarrow} \right)+ \sum_k \left( \Xi_k - E_k \right) - \frac{\Delta^2}{U_0/V}
\eeq
...
\eeq
\beq
\Delta \simeq 2 \epsilon_c e^{- \frac{1}{\rho(E_F)U_0}}
\eeq
...
\beq
\ket{\Phi} = \frac{1}{\sqrt{V}} \sum_k \phi_{1s} (k) c_k^\dagger h_{-k}^\dagger \ket{\Omega}
\eeq
...
\beq
\ket{\Psi} = \mathcal{N} \exp{\sum_k \lambda_k a_{ck}^\dagger a_{vk} }
\eeq
...
\beq
H_{XX} = \frac{1}{2A} \sum_{k, k', q}  V_q \psi^* _{X, k+q} \psi^*_{X, k'-q} \psi_{X, k} \psi_{X, k'}
\eeq
...
\beq
V_{dip} = - \frac{1}{2} \vv{d} \cdot\vv{E} \propto \mbox{Re}(\alpha) E^2
\eeq
...
\beq
\phi_n (\vv{p} ) = e^{i \vv{p} \cdot \vv{r}} u_n (\vv{r})
\eeq

\item[Low energy lattice Hamiltonian ] away from Feshbach resonance and at low temperatures, we have
\beq
H = - \sum_{\braket{ij}} J \left( b^\dagger_i b_j + b_j ^\dagger b_i\right)
\eeq
...
\beq
H = - J\sum_{\braket{ij}} \left( b^\dagger_i b_j + b_j ^\dagger b_i\right) - \sum_ \mu n_i + \frac{1}{2} \sum_i Un_i(n_i -1)
\eeq
...
\beq
H - \epsilon_F N = \sum_{\vv{k}} \bpmat c_{k \uparrow}^\dagger & c_{- k \downarrow} \epmat \bpmat \xi_k & - \Delta \\ - \Delta & - \xi_k \epmat \bpmat c_{k \uparrow} \\ c^\dagger_{- k \downarrow} \epmat + \sum_k \xi_k - \frac{\Delta^2}{V_0} \Omega
\eeq
where
\beq
...
\beq
\Delta = - \frac{V_0}{\Omega} \sum_k \bra{\psi} c_{- k\downarrow} c_{k \uparrow} \ket{+psi}
\eeq
...
\beq
2 \xi_k u_k v_k + \Delta ( v_k^2 - u_k^2) = 0
\eeq


\item[Diagonal Hamiltonian] is given by 
\beq
H - \epsilon_FN = \sum_{k \sigma} E_k \gamma_{k \sigma}^\dagger \gamma_{k \sigma} + \sum_k ( \xi_k  -E_k ) - \frac{\Delta^2}{V_0 } \Omega
\eeq
...
\begin{figure}[h]
  ...
  \centering
    \includegraphics[width=0.5\textwidth]{./SC_figures/Exciton_Spectrum.PNG}
\end{figure}
...
which gives
\beq
1 = \frac{V_0}{2U } \sum_k \frac{1}{\sqrt{\xi_k^2 + \Delta^2}}
\eeq

which we can solve for
\beq
\Delta \simeq 2 \omega_D e^{- \frac{1}{N(\epsilon_F) V_0}}
\eeq
...
\begin{figure}[h]
  ...
  \centering
    \includegraphics[width=0.5\textwidth]{./SC_figures/Vortex.PNG}
\end{figure}
...
\beq
I_c' = I_c - \left( \frac{I_b}{2} \pm I_s \right)
\eeq
...
\subsubsection{Voltage standard}
\begin{description}
...
\beq
H = E_C ( n - n_g)^2 - \left( \frac{E_J}{2} \sum_n \ket{n+1}\bra{n} + \ket{n}\bra{n+1} \right)
\eeq
...
\end{description}
\subsubsection{Conduction in nanowires}
\begin{description}
...
\begin{figure}[h]
  \caption{Phase slip.}
  \centering
    \includegraphics[width=0.5\textwidth]{./SC_figures/PhaseSlip.PNG}
\end{figure}
...
\end{description}
\subsection{Cryogenic Experimental Techniques}
\begin{description}
...
\subsubsection{Exclusion of EM noise}
\begin{description}
